...
de um artigo científico (menor que o de uma tese): 2-4 fontes validadas por
subtema, com 2 buscas por idioma como referência de esforço mínimo. Não decide
se um subtema "está bom" no sentido de conteúdo — só conta e compara contra o
limiar. Julgamento de qualidade/relevância continua com o LLM/agente.

Uso:
  python search_tracker.py --log research_log.json
  python search_tracker.py --log research_log.json --min-buscas 2 --min-fontes 2
...
def auditar(log: dict, min_buscas: int = 2, min_fontes: int = 2) -> dict:
    ...
    for subtema, dados in log["subtemas"].items():
        ...
        ok_lidas = n_fontes_lidas == n_fontes
    ...
    n_subtemas = len(log["subtemas"])
    if n_subtemas > 3:
        resultado["aviso_escopo"] = (
            f"{n_subtemas} subtemas é mais do que o recomendado (1-3) para um artigo — "
            "considere se o recorte não deveria virar mais de um artigo."
        )
...
def imprimir_relatorio(resultado: dict) -> None:
    ...
    for subtema, r in resultado["subtemas"].items():
        ...
        for p in r["pendencias"]:
            print(f"    - {p}")
    if resultado.get("aviso_escopo"):
        print(f"\nAVISO DE ESCOPO: {resultado['aviso_escopo']}")
    print()
...
def _cli():
    ap = argparse.ArgumentParser(description="Audita cobertura de busca vs. piso mínimo de artigo")
    ap.add_argument("--log", required=True, type=Path)
    ap.add_argument("--min-buscas", type=int, default=2)
